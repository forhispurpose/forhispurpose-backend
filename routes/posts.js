...
const ALLOWED_TYPES = ['devotional', 'article'];
...
router.get('/', async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('posts')
      .select('id, type, title, scripture_ref, body, author, published_at')
      .eq('type', type)
...
router.post('/', requireAdmin, async (req, res) => {
  const { type, title, body, scripture_ref, published_at } = req.body;
  if (!type || !ALLOWED_TYPES.includes(type)) {
    return res.status(400).json({ ok: false, error: 'type must be "devotional" or "article".' });
  }
